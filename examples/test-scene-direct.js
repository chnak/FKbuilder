 ...
 * 测试 Scene 单独渲染
 */
async function testSceneDirect() {
  console.log('=== 测试 Scene 单独渲染 ===');
  ...
  for (const layer of scene.timeline.getLayers()) {
    ...
    if (layer.elements) {
      ...
      for (const element of layer.elements) {
        console.log(`      元素: ${element.type}, startTime: ${element.startTime}, endTime: ${element.endTime}`);
      }
    }
  }
  ...
  await fs.writeFile('output/test-scene-direct.png', sceneBuffer);
  console.log('Scene canvas 已保存: output/test-scene-direct.png');
  
  scene.destroy();
}

testSceneDirect().catch(console.error);
